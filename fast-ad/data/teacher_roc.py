...
def load_scores(filepath):
    with h5py.File(filepath, "r") as f:
        scores = np.array(f["teacher_score"]).flatten().astype(np.float32)
...
def main():
    ...
    for i, filepath in enumerate(signal_files):
        ...
        scores = np.concatenate([sig_scores, zb_scores])
        labels = np.concatenate([
            np.ones(len(sig_scores)),
            np.zeros(len(zb_scores)),
        ])
    ...
    for i, filepath in enumerate(signal_files):
        ...
        # Only plot where fpr > 0 for log scale
        mask = fpr > 0
